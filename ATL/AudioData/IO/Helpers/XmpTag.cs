...
namespace ATL.AudioData.IO
{
    internal static class XmpTag
    {
        ...
        {
            IList<string> position = new List<string> { "xmp" };
            ...
            while (reader.Read())
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element: // Element start
                        ...
                        if (!key.Equals("x:xmpmeta", StringComparison.OrdinalIgnoreCase)) position.Add(key);
        ...
        public static int ToStream(BinaryWriter w, bool isLittleEndian, MetaDataIO meta)
        {
            ...
            w.Write(Utils.Latin1Encoding.GetBytes(CHUNK_XMP)); // TODO should be WAV-agnostic
