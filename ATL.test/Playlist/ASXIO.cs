                ...
                using (XmlReader source = XmlReader.Create(fs))
                {
                    while (source.Read())
                    {
                        if (source.NodeType == XmlNodeType.Element)
                        {
                            ...
                            else if (source.Name.Equals("ref", StringComparison.OrdinalIgnoreCase) && parents.Contains("entry")) Assert.AreEqual("file:///"+pathsToWrite[index].Replace('\\', '/'), source.GetAttribute("HREF"));
                        }
                    }
                }
                ...
                using (XmlReader source = XmlReader.Create(fs))
                {
                    while (source.Read())
                    {
                        if (source.NodeType == XmlNodeType.Element)
                        {
                            ...
                            {
                                if (source.Name.Equals("ref", StringComparison.OrdinalIgnoreCase)) Assert.AreEqual("file:///"+tracksToWrite[index].Path.Replace('\\', '/'), source.GetAttribute("HREF"));
